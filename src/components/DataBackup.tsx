...
const DataBackup: React.FC = () => {
  ...
  const exportData = () => {
    try {
      ...
      link.href = url;
      link.download = `kkn-budget-backup-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const importData = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setImportStatus({
          type: 'success',
          message: `Data imported successfully from backup created on ${new Date(importedData.timestamp).toLocaleDateString()}`
        });
      } catch (error) {
        console.error('Import failed:', error);
        setImportStatus({
          type: 'error',
          message: 'Invalid backup file or corrupted data. Please check your file and try again.'
        });
      }
    };
   ...
   */
  const clearAllData = () => {
    if (window.confirm('Are you sure you want to clear all data? This action cannot be undone. Make sure you have a backup first!')) {
      if (window.confirm('This will permanently delete all income, expense, and program data. Are you absolutely sure?')) {
        setState({
          user: null,
          incomes: [],
          expenses: [],
        ...
        setImportStatus({
          type: 'warning',
          message: 'All data has been cleared. You can restore from a backup if needed.'
        });
      }
    }
  };
    ...
    <Card className="w-full max-w-2xl mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        {importStatus.type && (
          <Alert 
            className={`${
              importStatus.type === 'success' ? 'border-green-200 bg-green-50' :
              importStatus.type === 'warning' ? 'border-yellow-200 bg-yellow-50' :
              'border-red-200 bg-red-50'
            }`}
          >
            {importStatus.type === 'success' && <CheckCircle className="h-4 w-4 text-green-600" />}
            {importStatus.type === 'warning' && <AlertTriangle className="h-4 w-4 text-yellow-600" />}
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm text-gray-600">
            Create a backup file of all your budget data. This file contains all income, expense, and program information.
          </p>
